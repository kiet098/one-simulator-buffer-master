...
public class sb_ICN_FIFO_SnW_SnW_Hybrid_Router extends ActiveRouter {
	/** identifier for the initial number of copies setting ({@value})*/ 
	public static final String NROF_COPIES = "nrofCopies";
	/** identifier for the binary-mode setting ({@value})*/ 
	public static final String BINARY_MODE = "binaryMode";
	/** SprayAndWait router's settings name space ({@value})*/ 
	public static final String ICN_HYBRID_NS = "sb_ICN_FIFO_SnW_SnW_Hybrid_Router";
	 ...
	 */
	public sb_ICN_FIFO_SnW_SnW_Hybrid_Router(Settings s) {
		super(s);
	 ...
	 */
	protected sb_ICN_FIFO_SnW_SnW_Hybrid_Router(sb_ICN_FIFO_SnW_SnW_Hybrid_Router r) {
		super(r);
	...
	public void update() {
		...
		List<Message> unsorted_response_list = new ArrayList<Message>();
		List<Message> unsorted_request_list = new ArrayList<Message>();
			...
			if(type.equals("data")){
				...
				//create a list of "response" this router is still carrying and nrofcopies >1
					if(m.getProperty(MSG_COUNT_PROPERTY) != null){
						...
						if(nrofCopies >1){
							unsorted_response_list.add(0,m);
						}				
					}
				}
				...
				//check if SnW property is added
				if(m.getProperty(MSG_COUNT_PROPERTY) == null){
					m.addProperty(MSG_COUNT_PROPERTY, new Integer(initialNrofCopies));
				}
		...
		List<Message> transferBuffer = sortByQueueMode(unsorted_response_list);
		
		//create a list of SAW "request" with copies left to distribute
		@SuppressWarnings(value = "unchecked")
		List<Message> requestBuffer = sortByQueueMode(unsorted_request_list);
			
		//add requestBuffer after the response buffer for transfer
		transferBuffer.addAll(requestBuffer);		
		
		//send all of SnW "request" and "response" with # of copies left > 1
	...
	protected void transferDone(Connection con) {
		...
		//check if it is response or request packet
		if(msg.isResponse() || type.equals("request")){
			if (msg == null) { // message has been dropped from the buffer after..
				...
				//the program throws null exception???
				//if(nrofCopies >1){
					if (isBinary) { 
						nrofCopies /= 2;
					}
	...
	@Override
	public sb_ICN_FIFO_SnW_SnW_Hybrid_Router replicate() {
		return new sb_ICN_FIFO_SnW_SnW_Hybrid_Router(this);
	}
