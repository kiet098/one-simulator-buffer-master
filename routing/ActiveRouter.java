...
public abstract class ActiveRouter extends MessageRouter {
	...
	public boolean requestDeliverableMessages(Connection con) {
		...
		/* do a copy to avoid concurrent modification exceptions (startTransfer may remove messages) */
	...
	public Message messageTransferred(String id, DTNHost from) {
		...
		if (m.getTo() == getHost() && m.getResponseSize() > 0) {
			...
			Message res = new Message(this.getHost(),m.getFrom(),RESPONSE_PREFIX+m.getId(), m.getResponseSize());
	...
	/**
	 * Makes rudimentary checks (that we have at least one message and one connection)
	 * about can this router start transfer.
	 ...
	 * this router (as final recipient), or DENIED_NO_SPACE if the message does not fit into buffer
	 */
	 ...
	 * @param size Size of the new message transferred, the transfer is aborted before message is removed
	...
	protected Tuple<Message, Connection> tryMessagesForConnected(List<Tuple<Message, Connection>> tuples) {
	  ...
	  * @return The message whose transfer was started or null if no transfer was started. 
	  */
	 ...
	 * @return The connections that started a transfer or null if no connection accepted a message.
	 */
	 ...
	 * @return A connection that started a transfer or null if no transfer was started
	 */
	...
	public void update() {		
		...
		if (SimClock.getTime() - lastTtlCheck >= TTL_CHECK_INTERVAL && sendingConnections.size() == 0) {
			dropExpiredMessages();
