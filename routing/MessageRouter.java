...
public abstract class MessageRouter {
	 ...
	 * values < 0 mean that the  receiving host will not accept this particular message (right now), 
	...
	//originally it is private HashMap<String, Message> deliveredMessages;
	 ...
	 * message listeners that need to be informed about message related events etc.
	 ...
	 * @return true if a message with the same ID has been received by this host as the final recipient.
	 */
	 ...
	 * copy of the collection should be made to avoid concurrent modification exceptions. 
	 ...
	 * @return True if the creation succeeded, false if not (e.g. message too big for buffer)
	 */
	 ...
	 * @return 
	 * -1 if the first message should come first 
	 * 1 if the second message should come first
	 * 0 if the ordering isn't defined
	 */
	...
	public RoutingInfo getRoutingInfo() {
		...
		RoutingInfo incoming = new RoutingInfo(this.incomingMessages.size() + " incoming message(s)");
		RoutingInfo delivered = new RoutingInfo(this.deliveredMessages.size() + " delivered message(s)");
		
		RoutingInfo cons = new RoutingInfo(host.getConnections().size() + " connection(s)");
